...
import {
  ...
  XCircle, ChevronUp, ChevronDown, Share2, PlayCircle, HelpCircle, Flag, BookOpen, Loader2
} from "lucide-react";
import { Movie, MovieGuessResult } from "@/types/movie";
import { movieData as staticMovieData } from "@/data/movieData";
...
function pickRandom(list: Movie[], exclude?: number): Movie {
  const pool = exclude !== undefined ? list.filter((m) => m.id !== exclude) : list;
...
function MovieAutocompleteInput({ onSelect, disabled, guessedIds, movieList }: MovieAutocompleteInputProps) {
...
export default function MovieWordle() {
  // ── Dynamic movie list (TMDB API) ────────────────────────
  const [movieList, setMovieList] = useState<Movie[]>(staticMovieData);
  const [loadingData, setLoadingData] = useState(true);
  const [target, setTarget] = useState<Movie | null>(null);

  useEffect(() => {
    let cancelled = false;
    fetch("/api/movies?action=all")
      .then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() as Promise<Movie[]>; })
      .then((data) => {
        if (cancelled) return;
        const list = Array.isArray(data) && data.length > 0 ? data : staticMovieData;
        setMovieList(list);
        setTarget(pickRandom(list));
      })
      .catch(() => { if (!cancelled) setTarget(pickRandom(staticMovieData)); })
  ...
  const reset = () => {
    setTarget(pickRandom(movieList, target?.id));
    setGuesses([]);
    setGameOver(false);
    setWon(false);
    setGaveUp(false);
